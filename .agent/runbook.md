...
| Referral signup (`/r/<code>`) gets stuck spinning forever on "Setting up your account" | Race condition in Auth.tsx: after `linkReferral()` RPC sets org_id in DB, `refreshProfile()` could return stale data (no org_id). Meanwhile `onAuthStateChange`'s deferred `fetchUserData()` (via setTimeout(0)) could overwrite fresh profile with stale data. The useEffect at line 359 waited for `profile?.org_id` which never arrived. | Changed both signup and existing-user referral flows in Auth.tsx to navigate directly after `linkReferral()` succeeds, instead of waiting for profile state. The RPC already confirmed org_id + created the contact; profile catches up via background `refreshProfile()`. |
