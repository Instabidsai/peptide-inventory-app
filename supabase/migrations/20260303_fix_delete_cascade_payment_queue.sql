-- Fix delete_contact_cascade: clear payment_email_queue.ai_suggested_contact_id
-- before deleting contact to avoid FK RESTRICT violation (HTTP 409)
...
BEGIN
  ...
  -- Clear payment_email_queue FK references (ai_suggested_contact_id has no ON DELETE)
  UPDATE payment_email_queue SET ai_suggested_contact_id = NULL
  WHERE ai_suggested_contact_id = p_contact_id;
  ...
  -- Delete the contact record before auth user
