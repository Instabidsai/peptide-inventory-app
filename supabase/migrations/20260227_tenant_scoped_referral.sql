-- Multi-tenancy fix: Add optional p_org_id to link_referral.
-- When a super_admin impersonates a tenant and shares a referral link,
-- the URL now includes the target org_id. If p_org_id is provided AND
-- that org exists, the new user joins that org instead of the referrer's.

-- Drop old 5-param overload first (CREATE OR REPLACE with new params creates a second overload)
DROP FUNCTION IF EXISTS public.link_referral(uuid, text, text, uuid, text);
...
BEGIN
  ...
  IF p_org_id IS NOT NULL THEN
    ...
    ELSE
      -- Invalid org_id — fall back to referrer's org
