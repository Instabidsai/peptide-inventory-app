-- ============================================================================
-- CRITICAL FIX: Prevent admin/super_admin profiles from being overwritten
-- ============================================================================
-- ROOT CAUSE: Both auto_link_contact_by_email and link_referral RPCs blindly
-- UPDATE profiles SET org_id = ..., role = ... without checking if the user
-- is already an admin or super_admin. This allowed a referral link click or
-- contact email match to DOWNGRADE an admin to a client in a different org.
--
-- INCIDENT: On 2026-03-25, the platform super_admin's profile was overwritten
-- from admin/PureUSPeptide to client/PureChainAminos via link_referral.
--
-- FIX: Add guards to both RPCs that refuse to modify profiles belonging to
-- admin or super_admin users. Return an error instead of silently overwriting.
-- ============================================================================

-- ============================================================================
-- FIX 1: auto_link_contact_by_email — add admin/super_admin guard
-- ============================================================================
...
-- ============================================================================
-- FIX 2: link_referral — add admin/super_admin guard
-- ============================================================================
...
-- ============================================================================
-- FIX 3: ensure_customer_contact — add admin/super_admin guard
-- ============================================================================
...
BEGIN
  ...
  -- GUARD: Don't auto-create contacts for admin/super_admin users
  IF v_profile.role = 'admin' THEN
    RETURN jsonb_build_object('created', false, 'reason', 'admin_skip',
      'detail', 'Admin users do not need client portal contacts');
  END IF;
  ...
  IF v_is_privileged THEN
    RETURN jsonb_build_object('created', false, 'reason', 'super_admin_skip',
      'detail', 'Super admin users do not need client portal contacts');
  END IF;
