-- Fix: promote_contact_to_partner duplicate detection used full_name matching
-- which missed existing profiles when the same contact was promoted twice,
-- causing a unique constraint violation (HTTP 409) on profiles.user_id.
-- Changed to match on user_id = p_contact_id and added ON CONFLICT guard.
...
BEGIN
    ...
    -- Use contact UUID as a placeholder user_id (no actual auth user)
    -- Check if a profile already exists for this contact by user_id
    ...
    ELSE
        -- Profile exists — update role and parent in case they changed
        UPDATE profiles
        SET role = 'sales_rep',
