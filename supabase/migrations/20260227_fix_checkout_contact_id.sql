-- Fix: create_validated_order fails with NOT NULL violation on client_id
-- when p_contact_id is not provided by the frontend.
-- Auto-resolve the caller's contact record via linked_user_id fallback.
