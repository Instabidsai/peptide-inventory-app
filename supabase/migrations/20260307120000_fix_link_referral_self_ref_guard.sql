-- ============================================================================
-- FIX: link_referral self-reference guard
-- When a user clicks their OWN referral link, parent_rep_id was set to their
-- own profile ID, making them invisible in the partner hierarchy (neither
-- top-level nor child of anyone else). This adds a guard to set parent_rep_id
-- to NULL instead of self-referencing.
-- ============================================================================
...
BEGIN
  ...
  -- 3. Get the new user's profile ID BEFORE updating (for self-reference guard)
  SELECT id INTO v_new_profile_id FROM profiles WHERE user_id = p_user_id;

  -- 3b. Update new user's profile — guard against self-referencing parent_rep_id
  --     If referrer IS the same person, set parent_rep_id to NULL instead
  UPDATE profiles SET
    org_id = v_target_org_id,
    parent_rep_id = CASE WHEN v_referrer_id = v_new_profile_id THEN NULL ELSE v_referrer_id END,
    parent_partner_id = CASE
      WHEN v_is_partner AND v_referrer_id != v_new_profile_id THEN v_referrer_id
      WHEN v_is_partner AND v_referrer_id = v_new_profile_id THEN NULL
      ELSE parent_partner_id
    END,
    role = v_app_role,
