-- Fix: create_validated_order never called process_sale_commission, so orders
-- created via client checkout (useValidatedCheckout) or partner store NEVER
-- got commission records in the commissions table. The order row had a flat
-- commission_amount but no actual commission records for the full upline chain
-- (direct rep → parent → grandparent).
--
-- Root cause: RLS silently blocks client-side INSERT into commissions.
-- process_sale_commission is SECURITY DEFINER and bypasses RLS.
-- This RPC now calls it internally when commission_rate > 0.
--
-- Partner self-orders (v_commission_rate = 0) are skipped — no commissions
-- on wholesale self-purchases.
...
BEGIN
  ...
  -- Only for orders that should have commissions (customers, contacts with reps).
  -- Partner self-orders have v_commission_rate = 0, so they are skipped.
  -- process_sale_commission is SECURITY DEFINER and creates actual records in
  -- the commissions table — client-side inserts are silently dropped by RLS.
