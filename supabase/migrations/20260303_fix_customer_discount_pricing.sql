-- ============================================================================
-- FIX: Customer orders created via create_validated_order use avg_cost * 1.0
-- instead of retail_price * 0.80 — the 20% customer discount never applies.
--
-- Root cause: The customer branch sets v_pricing_mode='percentage' but doesn't
-- update v_multiplier (stays 1.0). The price calc then does avg_cost * 1.0 = 0,
-- which triggers the sanity fallback to full retail_price.
--
-- Fix:
--   1. In customer branch: set v_multiplier = LEAST(profile_mult, 0.80)
--      matching the frontend logic in store/utils.ts calculateClientPrice()
--   2. In price calc: customers use retail_price as base, not avg_cost
-- ============================================================================
...
BEGIN
  ...
  -- Fallback: look up caller's own contact record
  ...
  IF v_contact IS NOT NULL THEN
    IF v_contact.type = 'customer' THEN
      ...
      -- Frontend: Math.min(profile.price_multiplier || 0.80, 0.80)
      v_multiplier := LEAST(
