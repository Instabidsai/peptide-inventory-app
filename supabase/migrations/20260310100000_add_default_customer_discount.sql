-- Add org-level default customer discount to tenant_config.
-- Stored as percentage off retail (e.g., 20 = 20% off MSRP).
-- Replaces the hardcoded 0.80 multiplier in create_validated_order() and frontend calculateClientPrice().

ALTER TABLE tenant_config
ADD COLUMN IF NOT EXISTS default_customer_discount numeric NOT NULL DEFAULT 20
CHECK (default_customer_discount >= 0 AND default_customer_discount <= 100);

COMMENT ON COLUMN tenant_config.default_customer_discount IS
  'Default % discount off retail for customer store orders. 20 = 20% off MSRP. Each org sets their own.';

-- Update create_validated_order() to read from tenant_config instead of hardcoding 0.80
...
BEGIN
  ...
  -- Fetch org-level default customer discount (percentage off retail)
  SELECT COALESCE(default_customer_discount, 20)
  INTO v_default_discount
  FROM tenant_config
  WHERE org_id = v_org_id;

  -- Fallback if no tenant_config row exists
  v_default_discount := COALESCE(v_default_discount, 20);
  ...
  IF v_contact IS NOT NULL THEN
    IF v_contact.type = 'customer' THEN
      ...
      -- Customer discount: use org-level default (e.g., 20 = 20% off → multiplier 0.80)
      -- If their profile has an even lower multiplier, use that instead.
      v_multiplier := LEAST(
        CASE WHEN COALESCE(v_profile.price_multiplier, 0) > 0
             THEN v_profile.price_multiplier ELSE (1.0 - v_default_discount / 100.0) END,
        (1.0 - v_default_discount / 100.0)
      );
  ...
  LOOP
    ...
    ELSE
      -- Partners: pricing off cost (wholesale)
