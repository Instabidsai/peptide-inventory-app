-- ============================================================================
-- FIX: link_referral should accept a p_tier param so referral links can
-- specify which partner tier to assign (standard, senior, referral).
-- Default is 'standard' — the most common partner link type.
-- ============================================================================
  ...
  IF v_is_partner THEN
    ...
    IF v_tier_commission IS NULL AND v_tier_key != 'standard' THEN
      ...
      WHERE org_id = v_target_org_id
        ...
        AND active = true
      LIMIT 1;
      -- If we fell back to standard config, use standard tier key
