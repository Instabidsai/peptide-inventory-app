...
export function TopBar({ onMenuClick }: TopBarProps) {
  ...
  const roleLabel = isThompsonOverride ? 'Partner' : (userRole?.role ? userRole.role.charAt(0).toUpperCase() + userRole.role.slice(1) : '');
  const roleVariant = userRole?.role === 'admin' && !isThompsonOverride ? 'default' : userRole?.role === 'staff' ? 'secondary' : 'outline';
