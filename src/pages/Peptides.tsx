...
import { z } from 'zod';
import { Plus, Pencil, Trash2, FlaskConical, Search, ChartBar } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { usePeptideStats } from '@/hooks/use-peptide-stats';
...
export default function Peptides() {
  ...
  const [viewingStatsPeptide, setViewingStatsPeptide] = useState<Peptide | null>(null);
  ...
  return (
    <div className="space-y-6">
              ...
              <TableBody>
                {filteredPeptides?.map((peptide) => (
                  <TableRow
                    ...
                    onClick={() => setViewingStatsPeptide(peptide)}
                  >
                    ...
                    <TableCell>
                      <Badge variant={stats?.[peptide.id] ? 'default' : 'secondary'}>
                        {stats?.[peptide.id] || 0} Units
                      </Badge>
                    </TableCell>
                    <TableCell>
      ...
      {/* Financial Stats Dialog */}
      <Dialog open={!!viewingStatsPeptide} onOpenChange={() => setViewingStatsPeptide(null)}>
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>{viewingStatsPeptide?.name} Finance</DialogTitle>
            <DialogDescription>
              Financial analysis including acquisition and effective costs.
            </DialogDescription>
          </DialogHeader>
          {viewingStatsPeptide && (
            <FinancialStatsCard peptideId={viewingStatsPeptide.id} />
          )}
        </DialogContent>
      </Dialog>
    </div>
