...
export default function MovementWizard() {
  ...
  const toggleBottle = (bottle: Bottle) => {
    if (isBottleSelected(bottle.id)) {
      ...
      const isZeroCost = movementType === 'internal_use' || movementType === 'giveaway';
      setSelectedBottles([
        ...selectedBottles,
        {
          bottle,
          price: isZeroCost ? 0 : Number(bottle.lots?.cost_per_unit || 0)
        },
      ]);
    }
  };
  ...
  const totalPrice = selectedBottles.reduce((sum, sb) => sum + sb.price, 0);

  const canProceedStep1 = movementType !== null;
  const canProceedStep2 = selectedBottles.length > 0;
  const canSubmit = canProceedStep1 && canProceedStep2;

  const handleSubmit = async () => {
    if (!movementType || selectedBottles.length === 0) return;
